...
export default async function handler(req,res){
  ...
  try{
    ...
    const j=await r.json().catch(()=>({})),o=j?.data||j||{};
    return res.status(200).json({started_at:toIsoKst(o.broad_start||o.BROAD_START||''),title:String(o.station_title||o.title||'').trim()});
  }catch(_){return res.status(200).json({started_at:'',title:''})}
